...
function New(fn) {
    ...
    // 4. 通过new创建的每个对象将最终被[[Prototype]]链接到这个函数的prototype对象上。
    // 5. 如果函数没有返回对象类型Object 
    //（包含Function，Array，Date，RegExg，Error），
...
Array.prototype._reduce(callback, initVal) {
    ...
    while (k < len) {
        if (k in Obj) { accumulator = callback.call(undefined, accumulator, obj[k], k, obj) }
        k++
    }
    return accumulator;
}
...
function lazyload() {
    window.addEventListener('scroll', function ()) {
        ...
        const callback = entries => {
            entries.forEach(ele => {
                console.log(ele)
                if (ele.isIntersecting) { // 是否被观察到
                    const data_src = ele.target.getAttribute('data-src'); // 真实图片地址
...
window.addEventListener('scroll', function () {
    const clientHeight = document.documentElement.clientHeight,
          ...
          srollHeight = document.documentElement.scrollHeight
...
const load = () => {
    const total = 100000,
          once = 20, // 一次插入的数据
          loopCount = Math.ceil(total / once) // 需要插入的次数
    let count = 0 // 当前渲染的次数
...
function _render(vnode) {
    ...
    if (typeof vnode === 'string') {
        return document.createTextNode(vnodes)
    }
...
function quickSort(arr) {
    ...
    let pivot = arr[Math.ceil(arr.length / 2)],
        left = arr.filter((v, i) => (v <= pivot && i !== arr.length - 1)),
        right = arr.filter(v => v > pivot)
